...
struct modeset_output {
	...
	struct gbm_device* gbm; // gbm_device
};
...
static int modeset_create_fb_gbm(int fd, struct gbm_device* gbm, struct modeset_buf *buf)
{
	uint32_t dmabuf_handles[4] = {0}, dmabuf_pitches[4] = {0}, dmabuf_offsets[4] = {0}, dmabuf_fds[4] = {0};

	uint64_t cap;
	if (drmGetCap(fd, DRM_CAP_PRIME, &cap) ||
			!(cap & DRM_PRIME_CAP_EXPORT)) {
		printf("PRIME export not supported");
		return -1;
	}
...
static void modeset_destroy_fb(int fd, struct modeset_buf *buf)
{
	...
	if (drmIoctl(fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dreq) != 0) {
		fprintf(stderr, "[!] failed to destroy dumb (%d): %m\n", errno);
	}

	// TODO: close all dmabuf fd
				      ...
				      struct modeset_output *out)
{
	...
	/* gbm device(EGLNativeDisplayType) */
	out->gbm = gbm_create_device(fd);
	if (out->gbm == nullptr) {
		fprintf(stderr, "failed to create gbm device.\n");
		return -1;
	}
	...
	for (i = 0; i < 2; i++) {
		...
		ret = modeset_create_fb_gbm(fd, out->gbm, &out->bufs[i]);
		if (ret) {
